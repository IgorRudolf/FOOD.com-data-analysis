...
turkey_ingr<- italian_ingr


list_of_words<- c("'canadian'","'mexican'","'italian'","'indian'","'turkey'" )
list_of_frames<- list(canadian_ingr,mexican_ingr,italian_ingr,indian_ingr,turkey_ingr )

for (i in 1:length(pp_recipes$id)){
  
  #first let's check if required tag is present
...
list_recipes_tag<- list(c(), c(), c(), c(), c())
list_of_words<- c("'canadian'","'mexican'","'italian'","'indian'","'turkey'" )
...
#now we will use raw_interactions

column_names <- sprintf("%02d", 1:12)

months_popularities<- data.frame(
  unique= unique_tab$unique
)

months_popularities[column_names]<-0
