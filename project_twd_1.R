...
#20/węgle
merged_id_rating %>% mutate(number_of_sugar_group=ifelse(carbohydrates<100,carbohydrates %/% 20,5)) ->carbohydrates_20_division
carbohydrates_20_division %>% group_by(number_of_sugar_group) %>% summarise(mean_sugar_rating=mean(mean_rating))
...
# z kolei 10 najlepszych charakteryzuje to, że są one z niskiej grupy kalorycznej
arranged_four_interesting_groups_20_division %>% filter(count>50) %>% arrange(-mean_combined_rating)
